...
# ---------------------------------------------------------------------------
# 5b. Pothole and curb — the Chapter 9 (Laplace) hook
# ---------------------------------------------------------------------------
def pothole_curb(omega0: float = 1.5, gamma: float = 0.3):
    """The Chapter-9 opening animation. Same chassis as :func:`road_test`,
    but the road presets are the two shapes the inherited UC/VoP toolbox
    can't touch cleanly:

    * **SMOOTH** — ``g(t) = sin(omega t)`` (Ch 7/8 territory; the baseline
      where UC/VoP work fine).
    * **POTHOLE** — a periodic train of narrow Gaussian dips, a smoothed
      ``g(t) = -sum delta(t - t_k)``. Each dip kicks a fresh bounce; with
      light damping you see them superpose.
    * **CURB** — a short flight of tanh-smoothed steps climbing to a
      plateau, ``g(t) = sum u(t - t_k)``. Each step is its own
      from-rest transient stacked onto the new equilibrium.

    Switching presets auto-resets the chassis so every reaction is
    from rest. ``omega0`` is the natural bounce frequency, ``gamma`` the
    shock-absorber damping (same conventions as ``road_test``).
    """
    import marimo as mo
    import anywidget
    import traitlets
    ...
    class _PotholeCurb(anywidget.AnyWidget):
        ...
        function render({ model, el }) {
          el.innerHTML = `
            <div style="font:13px sans-serif;color:#dfe7f2">
              ...
              <div style="display:flex;gap:10px;margin-top:10px;align-items:center;flex-wrap:wrap;color:#cdd6e0">
                ...
                <label style="margin-left:8px">stiffness ω₀ <input type="range" min="0.8" max="3.0" step="0.05" data-k="omega0"> <span data-v="omega0" style="color:#7ce0d3"></span></label>
                <label>damping γ <input type="range" min="0.05" max="1.2" step="0.05" data-k="gamma"> <span data-v="gamma" style="color:#ffb968"></span></label>
          ...
          var omega0 = parseFloat(model.get('init_omega0'));
          var gamma  = parseFloat(model.get('init_gamma'));
          el.querySelector('[data-v="omega0"]').textContent = omega0.toFixed(2);
          el.querySelector('[data-v="gamma"]').textContent  = gamma.toFixed(2);
          ...
          var T_WIN_BEFORE = 4.0, T_WIN_AFTER = 1.5;
          var PX_PER_UNIT = 56, GROUND_PX = 190, BODY_OFFSET = 60;
    ...
    return mo.ui.anywidget(_PotholeCurb(
        init_omega0=float(omega0), init_gamma=float(gamma)))


# ---------------------------------------------------------------------------
# 5c. The hidden hillside in 3-D — exact equations as contours (Ch 3a hook)
# ---------------------------------------------------------------------------
def hillside_3d(height: int = 440):
    """A 3-D hillside whose contour lines are the solutions of an exact
    equation ``M dx + N dy = 0``.

    The hidden landscape ``F(x, y)`` is a sum of several Gaussian hills and
    pits — a bumpy, organic terrain rather than a single peak-and-basin.
    Its contours ``F = C`` are exactly the integral curves of the exact
    equation with ``M = F_x``, ``N = F_y``: walking one keeps your altitude
    constant, which is what ``dF = F_x dx + F_y dy = 0`` says.

    Rendered with Three.js (WebGL). Drag to orbit, scroll to zoom, click
    the surface to drop a hiker who walks the contour through that point at
    constant altitude. A translucent water plane marks the held altitude;
    a gradient arrow at the hiker stays perpendicular to the path; the
    contour is also projected onto a ground plane as the 2-D map. Returns
    an ``mo.ui.anywidget``; the animation Lab and Chapter 3a share it.
    """
    import marimo as mo
    import anywidget

    class _Hillside3D(anywidget.AnyWidget):
        _esm = r"""
        ...
        function render({ model, el }) {
          ...
          // --- the hidden landscape: a sum of Gaussian hills and pits -----
          var N = 110, EXT = 3.0, HSCALE = 1.1, BASE_Y = -1.75;
          ...
          function F(x, y) {
            ...
            for (var b = 0; b < BUMPS.length; b++) {
              var dx = x - BUMPS[b][0], dy = y - BUMPS[b][1];
              var a = BUMPS[b][2], sg = BUMPS[b][3];
              s += a * Math.exp(-(dx*dx + dy*dy) / (2*sg*sg));
            }
            return s;
          }
          function grad(x, y) {
          ...
          // pre-pass: altitude range for color normalisation + level clamps
          var FMIN = 1e9, FMAX = -1e9;
          for (var j = 0; j < N; j++) for (var i = 0; i < N; i++) {
            var f = F(gc(i), gc(j)); if (f < FMIN) FMIN = f; if (f > FMAX) FMAX = f;
          }
          ...
          for (var j = 0; j < N; j++) {
            for (var i = 0; i < N; i++) {
              ...
              var c = terrainColor(norm(f)); colors.push(c[0], c[1], c[2]);
            }
          }
          for (var j = 0; j < N-1; j++) {
            for (var i = 0; i < N-1; i++) {
          ...
          var contourPts = [], hikerIdx = 0, zlevelCur = 0;
          ...
          function buildContour(zlevel, seed) {
            zlevel = Math.max(FMIN + 0.05, Math.min(FMAX - 0.05, zlevel));
            zlevelCur = zlevel;
            ...
            if (contourPts.length > 3) {
              ...
              var tube = new THREE.TubeGeometry(curve, Math.min(800, v3.length*2), 0.022, 8, tr.closed);
          ...
          buildContour(FMIN + 0.62 * (FMAX - FMIN));
    ...
    return mo.ui.anywidget(_Hillside3D())
